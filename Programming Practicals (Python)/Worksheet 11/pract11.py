# Challenge #1 - Storage Upgrade Options
class Laptop:
    ram_options = {
        4: 0,
        8: 50,
    ...
    def ssd(self, new_ssd):
        if new_ssd in self.hard_drive_options:
            self._ssd = new_ssd

    def calculate_price(self):
        ram_price = self.ram_options[self.ram]
        ssd_price = self.hard_drive_options[self.ssd]
        total_price = self.base_price + ram_price + ssd_price
        return total_price

    def __str__(self):
        output = f"{self.brand} Laptop with {self.ram} GB RAM & {self.ssd} GB Storage"
...
def test_laptop():
    ...
    # Challenge #1 - Storage Upgrade Options
    laptop.ssd = 512
    print(f"Laptop's Storage is {laptop.ssd} GB")

    print(f"Laptop's price (with upgraded hard drive) is £{laptop.calculate_price()}")
